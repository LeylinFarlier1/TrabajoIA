...
- **Estado (v0.1.9):** telemetría integrada (contadores de FRED, tiempos de respuesta) y health-check operativo exponiendo estado del caché y rate limiter.
...
- **Acción:**
  ...
  - Crear `utils/metrics.py` con registro in-process exportable a Prometheus y exponer `/metrics` o handler MCP equivalente.
...
- **Estado (v0.1.9):** logger configurable (plain/JSON), registro de request-id por contexto, métricas in-process exportables y herramienta `system_health` que consolida observabilidad básica.
...
| **v0.1.9 (Release)** | 1 semana | Observabilidad cerrada (logger JSON, métricas, health-check) + telemetría de cliente FRED ✅ |
